<p align="center">
    <a href="https://sourga.me" target="_blank">
        <img src="gh-assets/header.png" alt="Zing Cover Image">
    </a>
</p>
...
<p align="center">
    ...
    <a href="https://github.com/cfoust/sour/releases"><img src="https://img.shields.io/github/downloads/cfoust/sour/latest/total?style=flat-square" alt="Zing releases" /></a>
    <a href="https://github.com/cfoust/sour/blob/main/LICENSE"><img src="https://img.shields.io/github/license/cfoust/sour?color=48AC75&style=flat-square" alt="Zing License Badge MIT" /></a>
</p>

## What is this?

Zing is a <a target="_blank" href="http://sauerbraten.org/">Cube 2: Sauerbraten</a> server that serves a fully-featured web-version of Sauerbraten (with support for mobile devices) in addition to accepting connections from the traditional, desktop version of the game. Zing is the easiest way to play Sauerbraten with your friends.

<a target="_blank" href="https://sourga.me/">Give it a try.</a>

## Installation

You can download an archive containing the Zing server and all necessary assets from [the releases page](https://github.com/cfoust/sour/releases). For now, only Linux and macOS are supported.

You can also install Zing via `brew`:
...
To run Zing, extract a release archive anywhere you wish, navigate to that directory, and run `./sour`. If you installed Zing with `brew`, just run `sour` in any terminal session.

Running `sour` will start a Zing server accessible to web clients at `http://0.0.0.0:1337` and to desktop clients on port 28785. In other words, you should be able to connect to the Zing server in the Sauerbraten desktop client by running `/connect localhost`.

By serving on `0.0.0.0` by default, the Zing server will be available to other devices on the local network at IP of the device running the Zing server.

## Configuration

Zing is highly configurable. When run without arguments, `sour` defaults to running `sour serve` with the [default Zing configuration](https://github.com/cfoust/sour/blob/main/pkg/config/default.yaml). You change Zing's configuration by providing the path to a configuration file to `sour serve`:

```bash
sour serve config.yaml
```

Zing can be configured using `.yaml` or `.json` files; the structure is the same in both cases.
...
Zing also supports merging configurations together.
...
- **Preserve the experience of playing the original game.** While it is possible that Zing may someday support arbitrary game modes, assets, clients, and server code, the vanilla game experience should still be available.
- **Be the best example of a cross-platform, open-source FPS.** Deployment of Zing on your own infrastructure with whatever configuration you like should be easy. Every aspect of Zing should be configurable.

## Architecture

Here is a high level description of the repository's contents:

- `pkg` and `cmd`: All Go code used in Zing and its services.
  - `cmd/sourdump`: A Go program that calculates the minimum list of files necessary for the game to load a given map.
  - `cmd/sour`: The Zing game server, which provides a number of services to web clients:
    - Gives clients both on the web and desktop client access to game servers managed by Zing.
